...
class PaddingSchemesTest(unittest.TestCase):
    ...
    def testOEAPVector1(self):
        ...
        # --------------------------------------------------------------------------------
        # Message:
        m     = a2b_hex(compat_bytes('d4 36 e9 95 69 fd 32 a7 c8 a0 5b bc 90 d3 2c 49'.replace(' ',''),'utf-8'))
        label = ""
        lhash = a2b_hex(compat_bytes("da 39 a3 ee 5e 6b 4b 0d 32 55 bf ef 95 60 18 90 af d8 07 09".replace(' ',""),'utf-8'))
        DB    = a2b_hex(compat_bytes("da 39 a3 ee 5e 6b 4b 0d 32 55 bf ef 95 60 18 90 af d8 07 09 00 00 00 00\
        ...
        seed  = a2b_hex(compat_bytes("aa fd 12 f6 59 ca e6 34 89 b4 79 e5 07 6d de c2 f0 6c b5 8f".replace(' ' ,''),'utf-8'))
        #dbmask = dbMask = MGF (seed , 107):
        dbmask= a2b_hex(compat_bytes("06 e1 de b2 36 9a a5 a5 c7 07 d8 2c 8e 4e 93 24 8a c7 83 de e0 b2 c0 46\
        ...
        seedMask = a2b_hex(compat_bytes("41 87 0b 5a b0 29 e6 57 d9 57 50 b5 4c 28 3c 08 72 5d be a9".replace(' ',''),'utf-8'))
        maskedSeed= a2b_hex(compat_bytes("eb 7a 19 ac e9 e3 00 63 50 e3 29 50 4b 45 e2 ca 82 31 0b 26".replace(' ',''),'utf-8'))

        #EM = maskedSeed maskedDB
        EM       = a2b_hex(compat_bytes("00 eb 7a 19 ac e9 e3 00 63 50 e3 29 50 4b 45 e2 ca 82 31 0b 26 dc d8 7d 5c\
    ...
    def testSHA1Vector(self):
        ...
        V0 = (b"", a2b_hex(compat_bytes("da39a3ee5e6b4b0d3255bfef95601890afd80709",'utf-8')))
        V1 = (compat_bytes("The quick brown fox jumps over the lazy dog", 'utf-8'), a2b_hex(compat_bytes("2fd4e1c67a2d28fced849ee1bb76e7391b93eb12",'utf-8'))) #ASCII encoding
        V2 = (b'The quick brown fox jumps over the lazy dog', a2b_hex(compat_bytes("2fd4e1c67a2d28fced849ee1bb76e7391b93eb12",'utf-8'))) #binary data
        #print("str => ", V2[0])
    ...
    def testPSSTestVector(self):
        ...
        m = a2b_hex(compat_bytes('85 9e ef 2f d7 8a ca 00 30 8b dc 47 11 93 bf 55\
        ...
        # mHash:
        mHash = a2b_hex(compat_bytes('37 b6 6a e0 44 58 43 35 3d 47 ec b0 b4 fd 14 c1\
        10 e6 2d 6a'.replace(" ", ""),'utf-8'))
        
        # salt:
        salt = a2b_hex(compat_bytes('e3 b5 d5 d0 02 c1 bc e5 0c 2b 65 ef 88 a1 88 d8\
        3b ce 7e 61'.replace(" ", ""),'utf-8'))
        
        # M':
        mPrime = a2b_hex(compat_bytes('00 00 00 00 00 00 00 00 37 b6 6a e0 44 58 43 35\
        3d 47 ec b0 b4 fd 14 c1 10 e6 2d 6a e3 b5 d5 d0\
        02 c1 bc e5 0c 2b 65 ef 88 a1 88 d8 3b ce 7e 61'.replace(" ", ""),'utf-8'))
        
        # H:
        H = a2b_hex(compat_bytes('df 1a 89 6f 9d 8b c8 16 d9 7c d7 a2 c4 3b ad 54\
        6f be 8c fe'.replace(" ", ""),'utf-8'))
        
        # DB:
        DB = a2b_hex(compat_bytes('00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00\
        ...
        # dbMask:
        dbMask = a2b_hex(compat_bytes('66 e4 67 2e 83 6a d1 21 ba 24 4b ed 65 76 b8 67\
        ...
        # maskedDB:
        maskedDB = a2b_hex(compat_bytes('66 e4 67 2e 83 6a d1 21 ba 24 4b ed 65 76 b8 67\
        d9 a4 47 c2 8a 6e 66 a5 b8 7d ee 7f bc 7e 65 af\
        50 57 f8 6f ae 89 84 d9 ba 7f 96 9a d6 fe 02 a4\
        d7 5f 74 45 fe fd d8 5b 6d 3a 47 7c 28 d2 4b a1\
        e3 75 6f 79 2d d1 dc e8 ca 94 44 0e cb 52 79 ec\
        ...
        # Encoded message EM:
        EM = a2b_hex(compat_bytes('66 e4 67 2e 83 6a d1 21 ba 24 4b ed 65 76 b8 67\
        d9 a4 47 c2 8a 6e 66 a5 b8 7d ee 7f bc 7e 65 af\
        50 57 f8 6f ae 89 84 d9 ba 7f 96 9a d6 fe 02 a4\
        d7 5f 74 45 fe fd d8 5b 6d 3a 47 7c 28 d2 4b a1\
        e3 75 6f 79 2d d1 dc e8 ca 94 44 0e cb 52 79 ec\
...
class Pkcs7PaddingTest(unittest.TestCase):
    ...
    def encodecode(self,text):
        _bytes = compat_bytes(text,'utf-8')
